...
class KuGouParser(BaseParser):
    ...
    lzkey = "DragonCE105C81E0104B029F1F0FED784EBFC0"
    ...
    def _extract_embedded_info(self, html_text: str) -> dict:
        """提取页面内嵌的歌曲信息"""
        # 尝试提取dataFromSmarty中的歌曲信息
        smarty_match = re.search(
            r"var dataFromSmarty\s*=\s*(\[.*?\]),", 
            html_text, 
            re.DOTALL
        )
        ...
        if smarty_match:
            try:
                smarty_data = json.loads(smarty_match.group(1))
    ...
    async def _parse_kugou_share(self, searched: Match[str]):
        ...
        async with AsyncClient(headers=headers, verify=False, timeout=self.timeout) as client:
            ...
            logger.debug(f"内嵌歌曲信息: {embedded_info}")
            ...
            page_author = title_match.group(2).strip()
            search_title = f"{page_title} - {page_author}"

            logger.debug(f"搜索歌曲: {search_title}")
            ...
            try:
                ...
                logger.error(f"歌曲搜索失败: {str(e)}")
                try:
                    songs = await self.search_songs(page_title_clean)
                except Exception as e:
                    logger.error(f"使用标题二次搜索失败: {str(e)}")
                    raise ParseException(f"使用标题二次搜索失败: {str(e)}")
            ...
            for song in songs:
                ...
                # 1. 优先匹配内嵌hash
                if embedded_info and "hash" in embedded_info:
                    if song.get("hash", "").upper() == embedded_info["hash"]:
                        best_match = song
                        break
                ...
                author_similarity = SequenceMatcher(
                    None, 
                    str(song.get("singer", "")).lower(), 
                    str(page_author).lower()
                ).ratio()
            ...
            # 获取最佳匹配歌曲数据
            id = best_match.get("n", 1)
            
            try:
                song_info = await self.search_songs(search_title_clean, n=id)
            except Exception as e:
                raise ParseException(f"歌曲信息获取失败: {str(e)}")
            ...
            # 构建额外信息
            extra = {
                "info": f"时长: {int(float(song_details.get('duration', 0)) // 60)}:{int(float(song_details.get('duration', 0)) % 60):02d}",
                "type": "audio",
                "type_tag": "音乐",
                "type_icon": "fa-music",
            }
